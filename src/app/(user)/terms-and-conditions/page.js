...
export default function PrivacyPage() {
  const [termsData,setFaqData]=useState()
 ...
 const fetchPages = async () => {
    ...
    try {
      const response = await axios.get(
        `${base_url}/pages/get/terms-conditions`
      );
    ...
    <div>
     <Suspense fallback={null}>
      ...
      <div className="px-4 sm:px-8 lg:px-24 xl:px-60 mx-auto my-16">
        {!loading && termsData?.map((section, index) => (
          <div key={index} className="mb-10">
            <h2 className="text-xl font-semibold mb-3 text-[#111]">{section.heading}</h2>
            <p className="text-gray-700 leading-7">{section.des}</p>
          </div>
        ))}
      </div>
    </div>
